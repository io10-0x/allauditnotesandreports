  ...
  )
{
  ...
  /*c 
    ...
    function getAssetsIn(address account) external view returns (VToken[] memory) {
        ...
        The key bit I want to address is the assembly which you see stores len at offset assetsIn and assetsIn is initiated as a memory variable and from our assembly course and general knowledge, we know that this variable will be stored in memory and the first 32 bytes of the memory variable will be the length of the array. There is a caveat here though because we know that when we encode a dynamic array with abi.encode for example, what you get is the offset to the length of the array , the length of the array and then the elements in the array. So you might be thinking that if we store len at assetsIn , we are rewriting the offset but that would be wrong. What is actually happening here is that what assetsIn is, is simply a memory pointer. So it gives us the memory location of where assetsIn is stored in memory. So if the assetsIn array was in slot 0x40, assets in is just a pointer to 0x40. At 0x40, what we have is the length of the array and this is what happens with all memory variables you see in solidity so it is important to keep that in mind.
...
# 18 AAVE HOLDS LIQUIDITY IN ATOKEN CONTRACTS NOT POOL CONTRACTS EDGE CASE

This was a key point I got from solodit when researching edge cases and may be important for any future audits you do that integrate aave. The issue was that the protocol attempted to get the balance of aToken's directly from the aave pool. see below:
...
Aave actually holds all liquidity in the AToken contracts, not the pool contracts. As a result, the liquidity check in the code block above will always fail when the variablePool references a live instance of an actual aave pool. This is due to the fact that the variablePool balance will be 0. So they key point is keeping mind where Aave holds liquidity so any protocol attempting to check the balance of the pool contract for tokens will always be 0 . You can view the full finding via the link below:

https://solodit.cyfrin.io/issues/m-03-size-uses-wrong-source-to-query-available-liquidity-on-aave-resulting-in-borrow-and-lend-operations-being-bricked-upon-mainnet-deployment-code4rena-size-size-git

# 19 USING SOLODIT TO FIND EDGE CASES

Whenever you have covered as much as you can during an audit and you are at that weird place where you end up jumping around different functions doing assumption analysis, a great tool that can help you discover issues is solodit. At this point of your audit, you already have a deep understanding of the codebase so you are now searching for different edge cases and ideas to break the code.

Solodit contains a lot of previous reports. All you need to do is filter for any keyword you want. One that i used in this audit in the new commit was i went on solodit and searched for aave and scrolled through a few reports that had to do with protocols that integrated aave features and it showed me a few quirks with integrating aave and actually led me to a medium finding about aave incentives not being considered by velvet which you can see in my findings related to velvet. I also searched for venus and it showed me a finding related to chainlink oracles where minAnswer and maxAnswer variables from aggregators can be manipulated which was a finding I reported as well. On looking at how this issue affected velvet, it led me to another finding about undervalued vault balances so i cant stress the importance of this. This should be done only after you feel you have exhausted all you can during your manual review. It doesnt only give you more findings for the contests you are doing but can give you edge cases you can use for future audits like the aave issue I detailed in point 18.

# 20 USING REKT TO GET FINDING IDEAS AND ALWAYS READING IDEAS FROM PREVIOUS EXPLOITS

# 21 REIMAGINING FORMULAE IN SOLIDITY

In solidity, it can be a bit confusing to interpret what a formula is trying to do. Lets look at an example.
...
This formula is meant to get the amount of currentvaultbalance that a user is supposed to get. So normally in math, to do this, we would do (_portfolioTokenBalance/totalSupply ) * currentVaultBalance a d this would make perfect sense. In solidity though, if we do this, _portfolioTokenBalance is always going to be smaller than total supply and we know that in solidity, there are no decimals so it will just round down to 0 which we dont want to happen. So instead, the formula is rearranged to what you see above. So whenever you see a formula in solidity, you need to try to interpret it to what it would look like in normal math and it can help your understanding a lot.

# 22 AFTER ASSESSING A FUNCTION, ALWAYS USE ZOOM OUT METHODOLOGY
...
If you used zoom out analysis, after assessing the unusedCollateralPercentage calculation, you would have them zoomed out and asked, if TokenBalanceLibrary::_getAdjustedTokenBalance happens, where unusedCollateralPercentage is used, will it still work as intended? This is the idea of zooming out. So you look at a function with a first pass or/and assumption analysis, , you then use the brain trigger "if x happens, does this function/variable still work as intended?". Another variation of the trigger is "if this function runs, will x still work as intended?"  You know your brain works very well with brain triggers. so this brain trigger is what fires the zoom out analysis. This is what zooming out means. With this brain trigger, thinking about what x could be will help you discover bugs and x can literally be anything. The best pointers to what x could be would simply be identifying a state variable in the function you are assessing and global searching for everytime this variable is used in the codebase. This will show you related functions that use the same state variable and then you can ask if function x runs, will my function still run as intended? if my function runs, will it stop function x from running? Also dont limit your thinking to what is in the codebase. Also think about x being anything that could happen with any external integrations that the codebase uses. x can even be anything that can happen on the chain being deployed on. This is how far you can zoom out. This is the core of the zoom out methodology. Use this brain trigger to figure out all the things that x could possibly be and see how many more bugs you will find.
...
This was a rookie mistake you made but now you cannot fall to this exploit again. The idea is that whenever you are reviewing a codebase, you should try to hunt for functions that do similar things. You will always find these in every codebase. 2 functions that aim to do similar things but usually in slightly different ways. In these scenarios, the functions will usually update the same variables. Your aim is to compare the similar functions and hunt for any differences between them like variables that aren't updated or functionality in one function that isnt in another. Doing this can help you spot a lot of easy bugs. See the finding you missed that performed this comparison:

Description
There does appear to be an inconsistency in how the tokensBorrowed counter is updated between the repay and directDebtRepayment functions:

```solidity
// In repay function
bool isTokenFullyRepaid = borrowManager.repayVault(_controller, repayData);
if (isTokenFullyRepaid) tokensBorrowed--; // @audit-poc it's decrementing after full repay, which is fine
// In directDebtRepayment function
// No equivalent check or decrement of tokensBorrowed @audit-poc in directDebtRepayment the counter is not used

```
The repay function decrements the tokensBorrowed counter when a token is fully repaid, but directDebtRepayment lacks this mechanism. This means that debt positions fully repaid through directDebtRepayment will still count toward the borrowed token limit. And MAX_BORROW_TOKEN_LIMIT is set to 5 by default and can't be greater than 20.

Over time, this could lead to an inflated tokensBorrowed count that doesn't reflect reality. Since the contract enforces a maximum limit (MAX_BORROW_TOKEN_LIMIT), with MAX_BORROW_TOKEN_LIMIT set to only 5 by default, this bug presents a high risk since just 5 full repayments using directDebtRepayment would completely block all further borrowing operations.
You can view the full finding at:https://cantina.xyz/code/8cf9c7a0-a7a6-446a-8577-1e2c254eb5a8/findings?severity=medium,high&created_by=0xTheBlackPanther&finding=231


# 24 WATCH OUT FOR ANY IF BLOCKS USED IN A LOOP - SINGLE ITERATION VS FINAL RESULT EXTENSION

To understand this, lets look at the finding that identified the bug:

Summary
The protocol attempts to handle math errors in IVenusPool::getAccountSnapshot by skipping the problematic asset during processing. However, due to improper loop handling, encountering such an error results in an infinite loop, leading to DoS for deposits and withdrawals.

Finding Description
In VenusAssetHandler::processAssets, the function iterates through the controller’s assets for the vault and processes them. If updateVarsWithSnapshot returns true, the asset should be skipped.
```solidity
  function processAssets(address comptroller, address account,
    address[] memory assets, uint assetsCount, AccountLiquidityLocalVars memory vars,
    TokenInfo memory lendInfo, TokenInfo memory borrowInfo)
    internal view returns (
      AccountLiquidityLocalVars memory, TokenInfo memory, TokenInfo memory)
  {
...
```
The issue arises because i is not incremented before the continue statement. This means that if updateVarsWithSnapshot returns true, the loop retries the same asset indefinitely, eventually causing an OOG error.

The function updateVarsWithSnapshot returns true when IVenusPool::getAccountSnapshot fails with a non-zero error code:
...
```
On analyzing one of the Venus Pools used by the protocol, vBNB, oErr != 0 can be triggered by any math error when computing account balances and the exchange rate.

Since processAssets is called during both deposits and withdrawals, an OOG error in this function results in a DoS for both operations. You can view the full finding at: https://cantina.xyz/code/8cf9c7a0-a7a6-446a-8577-1e2c254eb5a8/findings?severity=medium,high&created_by=silverologist&finding=290

Looking at this finding, it is very straightforward and it is shocking how you missed it but this is for 2 reasons. The first is that you didnt perform assumption analysis on almost any of these functions which is why you neded upo missing some straightforward stuff. The second is that you didnt pay attention to if blocks inside a for loop. This is where the bug here was and although you looked at it, your brain didnt trigger when you saw an if block in a for loop. If it did, you would immediately have looked deeper into it and seen the bug and this is why i am pointing it out now so that in future, whenever you see an if block inside a loop, you know that there is likely to be something wrong in terms of single iteration vs final result exploit pattern which i have covered in a previous audit notes file. 


# 25 BNB TOKEN ON ETH MAINNET DOES NOT ALLOW ZERO APPROVAL

This finding is rather straightforward. Most protocols due to usdt not allowing partial approval, qill usually reset allowances to 0 before setting them again to whatever they want the allowance to be. Some tokens dont allow zero approvals though like BNB on eth mainnet and this is a weird quirk to keep in mind as could come in handy in the future. Lets look at the finding below:
...
```
the safeApprove() function code in TransferHelper library will throw if the approval call throws.
...
```
However, the BNB token's contract on Ethereum mainnet (https://etherscan.io/address/0xb8c77482e45f1f44de1745f52c74426c631bdd52#code) has a unique implementation that explicitly rejects zero-value approvals:
```solidity
...
# 26 BIRTHDAY PARADOX , WATCH OUT FOR VARIABLES DEFINED WITHIN A FINITE SPACE AS THEY RISK COLLISIONS DUE TO THE BIRTHDAY PARADOX

To fully understand this, lets look at the finding:

Summary
The contract uses a modulo operation on a hash to map tokens to bit positions within a limited range (0-65,535), creating a significant probability of hash collisions as the number of tokens increases. This vulnerability follows the birthday paradox principle and could lead to incorrect token tracking and system malfunction.

Finding Description
In RebalancingConfig.sol, tokens are mapped to bit positions using the following code:

uint256 bitPos = uint256(keccak256(abi.encodePacked(token))) % 65536; // Hash to get a unique bit position in the range 0-65,535
The code attempts to create a unique mapping between tokens and bit positions by using a hash function followed by a modulo operation to constrain the result to a 16-bit space (65,536 possible values). However, this approach is vulnerable to the birthday paradox, which states that in a finite space, the probability of collisions increases much faster than intuitively expected.

Based on probability calculations:

With 40 tokens: ~1% chance of collision
With 100 tokens: ~7.3% chance of collision
With 200 tokens: ~26.2% chance of collision
With 300 tokens: ~49.6% chance of collision
When a collision occurs:

Multiple different tokens will map to the same bit position This will cause the system to incorrectly track token states

You can view the full finding at:
https://cantina.xyz/code/8cf9c7a0-a7a6-446a-8577-1e2c254eb5a8/findings/41

This finding details an issue where the token address is hashed and then a modulo operator on 65536 is applied on the hash. The issue occuers where 2 hashes can modulo down to the same number if enough tokens are processed and the idea is that there are millions of tokens and many portfolios that can be crearted so eventually, 2 different tokens even though they have different hashes, when the modulo 65536 is applied to them, they will produce the same result and the likelihood of this happening is quite high. What this means is that in Rebalancing::updateTokens, the index gotten from the hashing is stored in an array . See the full flow below:


```solidity
 uint256 bitPos = uint256(keccak256(abi.encodePacked(token))) %
                    65536; // Hash to get a unique bit position in the range 0-65,535
...
So if 2 token hashes collide, the indexes will be the same , the offset's will be the same so the same bit in the tokenBitMap array will be updated which is where the collision occurs and that is the core of the issue. So whenever you see any value being confined to a finite space like the way the hash is confined to 65536, you need to think about the chances of potential collision and what the implications of that could be.

The idea that a value being confined to a finite space increases the risk of collision comes from the birthday paradox. 

The birthday paradox is a famous probability puzzle that shows how surprisingly high the probability is that, in a group of people, at least two share the same birthday—even if the group is much smaller than 365 (the number of days in a non-leap year). Here’s a detailed breakdown:

 Problem Statement
The paradox asks: “In a room of n people, what is the probability p(n) that at least two people have the same birthday?”
Surprisingly, when n=23, the probability p(23) exceeds 50%.

 Understanding the Paradox
At first, it might seem that you’d need many more people to have a good chance of a shared birthday because there are 365 days. However, the paradox arises because you’re not comparing an individual’s birthday to just one other person—instead, you’re comparing every possible pair in a group, and the number of pairs increases rapidly as the group grows.

 Calculating the Probability
Step 1: Calculate the Complement
It’s easier to start by calculating the probability that no two people share a birthday (i.e., all birthdays are unique) and then subtract that from 1.

Let p'(n) be the probability that in a group of n people no one shares a birthday.

For the first person, there are no restrictions (probability = 1).

For the second person, the probability that they have a different birthday from the first is 364/365.

For the third person, the probability that they have a unique birthday compared to the first two is 363/365 and so on.

Thus, for n people:

p'(n) = (365/365) * (364/365) * (363/365) * (362/365) * ..... * (365-(n+1)/365)
​
 
This product represents the probability that each successive person does not share a birthday with anyone who has already been considered.

Step 2: Convert to the Desired Probability
Since we want the probability that at least two people share a birthday, we have:
p(n)=1−p′(n)

Numerical Example: n=23
Let’s compute the probability step by step for 23 people:

p'(23) = (365/365) * (364/365) * (363/365) * (362/365) * ..... * (365 -22/365)
​
 
A quick calculation gives (or you can use a calculator):
p′(23)≈0.4927
So, the probability that at least two people share a birthday is:
p(23)=1−0.4927≈0.5073 or 50.73%
This means that with just 23 people, there is over a 50% chance that two people share the same birthday.

 Intuition Behind the Result
While 23 might seem small compared to 365, consider the number of pairs formed among 23 people. The number of distinct pairs 
𝑃 in a group of n people is given by: 
P = N(n-1)/2
 
For 𝑛=23 :
P = 23*22/2 = 253 pairs

With 253 pairs, even if the chance that any one pair shares a birthday is small (about 1/365) the collective chance that at least one pair collides grows quickly.

 Applications and Implications
Cryptography:
The birthday paradox concept is used in cryptographic collision attacks. For instance, in a hash function with a 128-bit output, a collision can be found in roughly 2^64 attempts —a principle known as the birthday bound.


# 27 ALWAYS VALIDATE ALL INPUTS FOR FUNCTIONS ESPECIALLY EXTERNAL ONES

This is another rookie mistake and action that you took for granted that has come back to bite you. You need to always validate all inputs to all functions. In every function, look at all the inputs(parameters) being passed into it and so assumption analysis on each one because missing even 1 will lead to you missing a vulnerability. See the finding below:

Description
The PositionManagerAbstractAlgebra contract contains errors in tuple destructuring when retrieving values from the positions function of the INonfungiblePositionManager. These issues occur in three functions: _getTokensOwed, _getTicksFromPosition, and _getExistingLiquidity. Each function incorrectly skips or assigns return values from the positions call, resulting in misaligned indices that lead to improper fee, tick, and liquidity extraction.

In the _getTokensOwed function, the code is written as follows:

(, , , , , , , , , tokensOwed0, tokensOwed1) = INonfungiblePositionManager(
  address(uniswapV3PositionManager)
).positions(_tokenId);
The positions function returns twelve values in the order: nonce, operator, token0, token1, fee, tickLower, tickUpper, liquidity, feeGrowthInside0LastX128, feeGrowthInside1LastX128, tokensOwed0, tokensOwed1. With the current tuple destructuring, only nine values are skipped, which misassigns tokensOwed0 to the value at index 9 (feeGrowthInside1LastX128) and tokensOwed1 to index 10 (tokensOwed0). This misalignment prevents the function from retrieving the correct uncollected fee amounts.

The _getTicksFromPosition function currently extracts ticks as follows:

(, , , , tickLower, tickUpper, , , , , ) = INonfungiblePositionManager( ... ).positions(_tokenId);
Given the expected return order, tickLower should be at index 5 and tickUpper at index 6. However, without an extra placeholder to skip the fee value at index 4, the current implementation mistakenly assigns tickLower from index 4 (fee) and tickUpper from index 5 (tickLower), thereby shifting the intended values.

Similarly, the _getExistingLiquidity function uses this extraction:

(, , , , , , existingLiquidity, , , , ) = INonfungiblePositionManager(
  address(uniswapV3PositionManager)
).positions(_tokenId);
Liquidity is actually the eighth return value (index 7) after nonce, operator, token0, token1, fee, tickLower, and tickUpper. Without including an additional placeholder to bypass the tickUpper value, the current code incorrectly assigns existingLiquidity to the tickUpper value. These errors in tuple destructuring directly lead to the wrong values being used in critical operations within the contract.

View the full finding at:
https://cantina.xyz/code/8cf9c7a0-a7a6-446a-8577-1e2c254eb5a8/findings?status=new,duplicate,confirmed,acknowledged,fixed&severity=medium,high&created_by=0xBeastBoy&finding=277


# 28 IMMUTABLE AND CONSTANT DECLARATIONS
It is very important whenever you see immutable or constant declarations, you need to query whether these variables will need to be changed at any point because often, projects forget that a variable may need to be changed and either declare as immutable or constant so it is important to make those checks. Not doing this caused you to miss out on a finding. See below:

Summary
The PRICE_ORACLE_ADDRESS is hardcoded for all network deployments. The PRICE_ORACLE_ADDRESS is not deployed to the same address on all chains and will cause calls to (PRICE_ORACLE_ADDRESS).getAssetPrice(...) to revert which in turn causes denial of service on the executeOperation(...) function call on all other chains apart from Arbitrum.
...
You can view the full finding at : https://cantina.xyz/code/8cf9c7a0-a7a6-446a-8577-1e2c254eb5a8/findings?status=&q=base&finding=491


# 28 OUT OF GAS ERROR WITH WETH DUE TO ATTEMPTING TO WRITE TO STORAGE DURING STATIC CALL

This was a really cool finding I saw and i think it is worth knowing and it has to do with how if you attempt to write to storage during a static call (read only or view function), it actually doesnt revert but causes an out of gas error. The finding explains it best so lets look into it:

The AaveAssetHandler uses the getAllProtocolAssets function to retrieve all lending or borrowing tokens. These tokens are then used in BorrowManagerAave.executeOperation to process loans, perform swaps, transfers, withdrawals, and repayments.

The issue occurs when the portfolio includes WETH as a token. Due to WETH's fallback function, the following code will not revert as expected:

for (uint i = 0; i < portfolioTokensLength; i++) {
  //@audit When portfolioTokens[i] == WETH it won't revert
  try IAaveToken(portfolioTokens[i]).UNDERLYING_ASSET_ADDRESS() {
    lendTokens[lendCount++] = portfolioTokens[i];
  } catch {}
}
As a result, WETH will be incorrectly classified as a lending token (aToken) despite not being one, which will cause calculation errors.

Here is a comment from our internal review which calls to getPool function:

Indeed the try/catch clause is here to handle the revert in the case where the function getPool does not exist on the token; But in the case of WETH9 the fallback function is executed which attempts a deposit.

As a result, an EVM error (attempt to write to storage during staticcall) is thrown instead of a revert, and consumes all of the remaining gas in the context.

Here is some output of my test:

testRevertDeposit() (gas: 992793222)

Gas Before WETH call to `UNDERLYING_ASSET_ADDRESS()`: 1007523208

Gas After WETH call: 15742425
Note that due to EIP-150, only 63/64 of gas is passed on to the IAaveToken(portfolioTokens[i]).UNDERLYING_ASSET_ADDRESS(), so the call can technically be made to succeed by wasting an enormous amount of gas.

Considering gas price at 30 gwei, the total cost of this call is 992793222 * 30 * 10⁻⁹ = 29.78379666 ETH to make the deposit. This is an interesting finding that would be difficult to discover during testing, as it only becomes apparent when analyzing the gas consumption of function calls.

Gist of the test: https://gist.github.com/jonatascm/1bd1d6b911fe40bf8ec987d421c68600
You can view the full finding at:
