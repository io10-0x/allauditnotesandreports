...
The formula typically used to calculate liquidity index was used in RAAC in the ReserveLibrary.sol contract and I will display it with the comments I left on it so you can see how it works:
...
```solidity
  function calculateLinearInterest(
    ...
    function calculateLiquidityIndex(
        ...
        //c so the cummulated interest is multiplied by the old liquidity index to get the new liquidity index. This is because The liquidity index stores a historical record of accrued interest, and by multiplying it with cumulatedInterest, we ensure correct compounding over time. The reason why we multiplied instead of adding is detailed below
...
The global bias and the global scope will have to change whenever the total supply function is called for a number of reasons. the obvious one is that total supply is also going to reduce over time due to the decay mechanics. For the global scope specifically, the main reason is because when a user's lock ends, their slope doesnt contribute to the amount of tokens that all users have lost up to that point anymore because they have no voting power anymore so for them, their point.bias - (point.slope times (block.timestamp - point.timestamp)) is 0.

So for our total supply calculation, we need to reflect this because if we dont, the rate of decay will be more than it needs to be because we are including the rate of decay of a user who has no voting power which doesnt make sense. So we need to find a way to remove a user's slope from the global scope once their voting power is 0 i.e. at the end of their lock time. We also need to map out how to reduce global bias over time.

To reduce global scope, we need to track the amount of slope we need to subtract from global scope at what time. To adjust global bias, we will also need to keep track of the timestamp the total supply function There are 3 things that will allow us do this:
...
A checkpoint that stores the current timestamp everytim the total supply function is called.
...
≈ 160 - 60
= 100
update lastCheckpointTimestamp = 604800
update globalBias = 100
...
lastCheckpointTimestamp = 604800
totalSupply = globalBias - globalSlope _ (block.timestamp - lastCheckpointTimestamp)
= 100 - 0.00009921 _ 604800
≈ 100 - 60
= 40
globalBias = 40
...
Notice how the global scope is only updated after the total supply and global bias have been updated. the cached values are used to calculate the current total supply prior to any updates being made.
...
As we as using voting escrows (ve mechanics), RAAC implemented the use of a DAO which is something we covered during the foundry course so you can go over that for a recap but here we are going to cover some key things you need to look out for whnever you see voting escrows or DAO's being used in protocols.
