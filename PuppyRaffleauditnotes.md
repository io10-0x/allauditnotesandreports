...
I ran into an issue when testing this both in brownie and remix, i ran into an issue. the issue was that in my batch call, i tried to pass 10 calls into the calls array to call
commiteth 10 times after having some people commitEth to the auction to see if i could take their eth using the msg.value exploit and when i ran it like this, everything worked perfectly.
Keep in mind that i passed a true boolean to the batch function which meant that if there were any reverts in any of the calls, the function would revert. See the batch function:
...
As a result, if any of the calls in the calls array failed, the transaction would revert and this caused an issue because for some reason, when i increased the array to 11 commiteeth calls,
the final one fails and since I am using brownie to test, it was very hard to see why this was happening. It got me spiralling for a bit to figure out why. The issue has something to do with
the transfer function used in the commiteth function. This function is very different from the call function and this was my first indication of it because i changed that transfer function to
a low level call and everything worked even with the true boolean set in the batch function. I covered more on this transfer function vs low level called in the notes.md in the ChristmasDinner
directory so have a look there to know more.

Anyway, it was causing one of the commiteth functions to fail so i decided to change the revertonfail function to false because i wanted the batch function to run all of the calls in the array
regardless of if one failed or not because i knew there was almost no chance that all of them would fail so all the ones that passed would still help me get all of the eth out of the contract.
So i changed that the revertonfail boolean to false and all transactions worked as expected. Just to make sure it wasnt an environment issue, i redid the test on remix and found the same issue
with the same fix so it confirmed that my suspicion was right. You can go into the MISO directory and see the test_dutch_auction_canbatchcommitethwithsamemsgvalue test i wrote in the test_dutch_auction file.
